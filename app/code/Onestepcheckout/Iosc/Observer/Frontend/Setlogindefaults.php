...
class Setlogindefaults implements \Magento\Framework\Event\ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if (!empty($quote->getCustomer()) && !empty($quote->getCustomer()->getAddresses())) {
            ...
            if (!$quote->getBillingAddress()->getCustomerAddressId() && (is_object($primaryBilling) && $primaryBilling->getId())) {
                $newBilling = $this->copyAddressData($customer->getPrimaryBillingAddress(), $quote->getBillingAddress());
                $quote->setBillingAddress($newBilling);
                $saveQuote = true;
            }

            $primaryShipping = $customer->getPrimaryShippingAddress();
            if (!$quote->getShippingAddress()->getCustomerAddressId() && (is_object($primaryShipping) && $primaryShipping->getId())) {
                $newShipping = $this->copyAddressData($customer->getPrimaryShippingAddress(), $quote->getShippingAddress());
