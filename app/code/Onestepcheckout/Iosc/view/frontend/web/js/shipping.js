    ...
    function (
        ...
        return uiComponent.extend({
            initialize : function () {
                ...
                if (isSafari) {
                    checkoutDataResolver.resolveShippingRates = _.wrap(
                ...
                newAddress.getRates = _.wrap(
                    newAddress.getRates,
                    this.voidFunc.bind(this)
                );

                customerAddress.getRates = _.wrap(
                ...
                uiRegistry.async("checkout.steps.shipping-step.shippingAddress")(
                    function (shippingView) {
                        ...
                        shippingView.selectShippingMethod = _.wrap(shippingView.selectShippingMethod, function (originalMethod, shippingMethod, e) {
                            e.stopImmediatePropagation();
                            var originalReturn = originalMethod(shippingMethod);
                            shippingSaveProcessor.saveShippingInformation();
                            return originalReturn;
                        });
                        shippingView.getPopUp = _.wrap(
            ...
            successHandler: function (data) {
                ...
                if (typeof data.data.shippingMethod !== "undefined" && data.data.shippingMethod.length > 0) {
                    ...
                    if (this.rateCacheKey) {
                        equal = _.isEqual(rateRegistry.get(this.rateCacheKey),data.data.shippingMethod);
                    }

                    if (!equal) {
                        rateRegistry.set(this.rateCacheKey, data.data.shippingMethod);
                        shippingService.setShippingRates(data.data.shippingMethod);
                    } else if (equal && this._has(data, "data.shippingMethod")) {
                        shippingService.setShippingRates(data.data.shippingMethod);
                    }

                    if (this._has(data, "data.selectedShippingMethod") && data.data.selectedShippingMethod === "") {
                        quote.shippingMethod(null);
                    }
                } else {
                ...
                uiRegistry.async("checkout.steps.shipping-step.shippingAddress")(
                    function (paymentStep) {
                        domObserver.get("div.checkout-shipping-method div.step-title", function (elem) {
                            jQuery(elem).prepend(jQuery("<span class='title-number'><span>" + jQuery.mage.__("2") + "</span></span>").get(0));
                        });

                        domObserver.get("li.checkout-shipping-address div.step-title", function (elem) {
                            jQuery(elem).prepend(jQuery("<span class='title-number'><span>" + jQuery.mage.__("1") + "</span></span>").get(0));
                        });
