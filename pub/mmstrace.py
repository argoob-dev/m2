...
if stratus == '1':
   ...
   #BUILD STRACE CMD
   print ""
   finalstrace = shttp + " HTTP_HOST=" + surl + " REQUEST_METHOD=GET" + " SERVER_ADDR=" + intip + " REMOTE_ADDR=" + intip + " REQUEST_URI=" + spage + " SERVER_NAME=" + surl + " strace -r -s 1024 -tt -e trace=sendto,connect,open,write php index.php 1>/dev/null 2>mmstrace/strace.out"
   print finalstrace
   print ""
   ...
   #SLOW PATHS TOP 5
   pathsslow = "grep '/srv/' mmstrace/strace.out | sort -n | tail -5 > mmstrace/strace_paths_slow"
   os.system(pathsslow)
   ...
   #BUILD STRACE CMD
   finalstrace = shttp + " HTTP_HOST=" + surl + " REQUEST_METHOD=GET" + " REQUEST_URI=" + spage + " SERVER_NAME=" + surl + " strace -r -s 1024 -tt -e trace=sendto,connect,open,write " + sphp + " index.php 1>/dev/null 2>mmstrace/strace.out"
   print " "
   print finalstrace
   ...
   #SLOW PATHS TOP 5
   pathsslow = "grep '/home/' mmstrace/strace.out | sort -n | tail -5 > mmstrace/strace_paths_slow"
   os.system(pathsslow)


#SLOW QUERIES / COUNT ALL QUERIES
querycmd = "grep 'sendto' mmstrace/strace.out | sort -n | tail -5 > mmstrace/strace_queries"
#spacecmd = "echo ' ' >> mmstrace/strace_queries"
qtagcount = "echo -n ' queries total = ' >> mmstrace/strace_queries"
querycount = "grep 'sendto' mmstrace/strace.out | wc -l >> mmstrace/strace_queries"
os.system(querycmd)
#os.system(spacecmd)
os.system(qtagcount)
...
if os.path.exists("../bin/magento"):
   ...
   modcmd = "n98-magerun2 dev:module:list | grep -v Magento >> mmstrace/strace-notable.out"   
...
info = "echo 'This section greps for keywords of interest: error, exception, connection' > mmstrace/strace_errors.out"
errcmd = "grep error mmstrace/strace.out >> mmstrace/strace_errors.out"
errcmdb = "grep exception mmstrace/strace.out >> mmstrace/strace_errors.out"
errcmdc = "grep connection mmstrace/strace.out >> mmstrace/strace_errors.out"
os.system(info)
os.system(errcmd)
os.system(errcmdb)
os.system(errcmdc)
